                     ...
                     max.dist = 1e6)
{
  ...
  ## multiple haplotypes for complex graph with bubbles)  
    ...
    dt.alt[, is.min := rep(is.element(seq_len(.N), which.min(val)), length.out = .N), by = .(qid, sid)]
    
    dt.alt = dt.alt[is.min == TRUE, ]

    ## dt.ref[, is.min := 1:.N %in% which.min(val), by = .(qid, sid)]
    dt.ref[, is.min := rep(is.element(seq_len(.N), which.min(val)), length.out = .N), by = .(qid, sid)]
